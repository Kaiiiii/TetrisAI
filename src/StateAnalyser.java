...
public class StateAnalyser {
    private static final int COUNT_METRICS = 5;
    ...
    private static final int INDEX_METRIC_LANDING_HEIGHT = 4;
    ...
    public AnalysisResult analyse(int move) {
        ...
        int pieceHeight = State.getpHeight()[this._state.getNextPiece()][orient];

        // Find out the base
        int base = IntStream.range(0, pieceWidth).map(index -> this._state.getTop()[column + index]
                - State.getpBottom()[this._state.getNextPiece()][orient][index]).max().orElse(0);

        // Get new tops/heights
        final int[] newTop = new int[pieceWidth];
        IntStream.range(0, pieceWidth).forEach(index -> {
            newTop[index] = State.getpTop()[this._state.getNextPiece()][orient][index] + base;
        });
        ...
        metrics[INDEX_METRIC_HOLES] = IntStream.range(0, pieceWidth).map(index -> base
                + State.getpBottom()[this._state.getNextPiece()][orient][index] - this._state.getTop()[column + index])
                .sum();
        ...
        for (int col = 0; col < State.COLS; col++) {
            ...
            int currentColHeight = (col < column || col >= column + pieceWidth) ? this._state.getTop()[col]
                    : newTop[col - column];
            ...
            // HOLES
            // The holes of the special columns were already taken into account
            // above
            // we only count the rest here
            metrics[INDEX_METRIC_HOLES] += IntStream.range(0, this._state.getTop()[currentCol]).parallel()
                    .filter(row -> this._state.getField()[row][currentCol] == 0).count();

            // BUMPINESS
            // If the previous col height is null, then there isn't any value
            // yet,
        ...
        // If base row is below the lowest row, there is none, so we don't
        // consider this case
        // We will try to measure the rows in which they are both affected by
        // the placing, and
        // also higher than the lowest row
        for (int row = base; row < lowestRow; row++) {
            final int currentRow = row;
            // Filter out unaffected columns, if they contains an empty cell
            // then we can skip
            // right away
            int emptyCells = (int) IntStream.range(0, State.COLS).parallel()
                    .filter(col -> col < column || col >= column + pieceWidth)
                    .filter(col -> this._state.getField()[currentRow][col] == 0).count();
            if (emptyCells > 0) {
                continue;
            }
        ...
        metrics[INDEX_METRIC_LANDING_HEIGHT] = getLandingHeight(newTop, pieceHeight);

        return new AnalysisResult(move, metrics[INDEX_METRIC_AGGREGATE_HEIGHT], metrics[INDEX_METRIC_COMPLETE_LINES],
                metrics[INDEX_METRIC_HOLES], metrics[INDEX_METRIC_BUMPINESS], metrics[INDEX_METRIC_LANDING_HEIGHT]);
    }

    private int getLandingHeight(int[] newTop, int pieceHeight) {
        int maxTop = 0;
        ...
        int result = maxTop - ((pieceHeight - 1) / 2);
